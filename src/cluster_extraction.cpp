...
void ClusterExtraction::onInit() {
  tf_listener = std::shared_ptr<tf::TransformListener>(
      new tf::TransformListener(nh_, ros::Duration(10)));

  cloud_sub =
      nh_.subscribe("points", 3, &ClusterExtraction::cloudCallback, this);

  clusters_pub = nh_.advertise<cluster_extraction::ClusterArray>("clusters", 1);

  c_e_thread_id_ =
      std::thread(std::bind(&ClusterExtraction::clusterExtractionThread, this));

  NODELET_INFO("Cluster Extraction has started!");
}

void ClusterExtraction::clusterExtractionThread() {
  subscribed_ = false;
  ...
  while (ros::ok()) {
    ...
    ros::param::get("/cluster_extraction_enable",
                    cluster_extraction_enable_param);
...
void ClusterExtraction::cloudCallback(
    const sensor_msgs::PointCloud2ConstPtr& _cloud) {
  pcl_data = _cloud;
}
...
void ClusterExtraction::processCloud(float plane_tolerance) {
  ...
  tf::StampedTransform world_to_openni;

  try {
    tf_listener->waitForTransform("world", cloud->header.frame_id, ros::Time(0),
                                  ros::Duration(1));
    // tf_listener->transformPoint("world", plane_normal, _plane_normal);
    tf_listener->lookupTransform("world", cloud->header.frame_id, ros::Time(0),
                                 world_to_openni);
  } catch (tf::TransformException& ex) {
    ROS_INFO("COCKED UP POINT INFO! Why: %s", ex.what());
  }

  // We do this here so that we can put in an empty cluster, if we see no table
  // in the first place.
  cluster_extraction::ClusterArray __clusters;
  ...
  while (cloud->points.size() > 0.5 * nr_points) {
    ...
    tf::Vector3 plane_normal(coefficients->values[0], coefficients->values[1],
                             coefficients->values[2]);
    tf::Vector3 _plane_normal = world_to_openni * plane_normal;

    // What's the angle between this vector and the actual z axis? cos_inverse (
    // j component )...
  ...
  tf::Vector3 _plane_centroid = world_to_openni * plane_centroid;

  geometry_msgs::PointStamped _plane_centroid_ROSMsg;
  _plane_centroid_ROSMsg.header.frame_id = "world";
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it =
    ...
    computeClusterDimensions(cloud_cluster, a, b);
    ...
    tf::Vector3 cluster_centroid(cluster_cen[0], cluster_cen[1],
                                 cluster_cen[2]);
    tf::Vector3 _cluster_centroid = world_to_openni * cluster_centroid;

    geometry_msgs::PointStamped _cluster_centroid_ROSMsg;
    _cluster_centroid_ROSMsg.header.frame_id = "world";
    ...
    if (DIST(cluster_centroid, plane_centroid) < 0.6 &&
        _cluster_centroid.z() > _plane_centroid.z()) {
      cluster_extraction::Cluster __cluster;
      __cluster.centroid = _cluster_centroid_ROSMsg;
      // Push cluster dimentions. Viewed width, breadth and height
      __cluster.min_point = a;
      __cluster.max_point = b;
      // Push colors
...
std::vector<double> ClusterExtraction::computeClusterDimensions(
    const pcl::PointCloud<PoinT>::ConstPtr& input_cluster,
    geometry_msgs::Point& a, geometry_msgs::Point& b) {
  tf::StampedTransform world_to_openni;

  try {
    tf_listener->waitForTransform("world", input_cluster->header.frame_id,
                                  ros::Time(0), ros::Duration(1));
    tf_listener->lookupTransform("world", input_cluster->header.frame_id,
                                 ros::Time(0), world_to_openni);
  } catch (tf::TransformException& ex) {
    ROS_INFO("COCKED UP POINT INFO! Why: %s", ex.what());
  }
  ...
  BOOST_FOREACH (const PoinT& pt, input_cluster->points) {
    ...
    tf::Vector3 _pt_transformed_ = world_to_openni * _pt_;
    PoinT thisPoint;
  ...
  double min_X = transformed_cloud.points[0].x,
         min_Y = transformed_cloud.points[0].y,
         min_Z = transformed_cloud.points[0].z,
         max_X = transformed_cloud.points[0].x,
         max_Y = transformed_cloud.points[0].y,
         max_Z = transformed_cloud.points[0].z;
...
int ClusterExtraction::getCylinders(
    const pcl::PointCloud<PoinT>::ConstPtr& _cloud) {
  ...
  *cloud = *_cloud;
  ...
  pcl::PointCloud<PoinT>::Ptr cloud_f(new pcl::PointCloud<PoinT>);
  pcl::PointCloud<pcl::Normal>::Ptr cloud_norm_f(
      new pcl::PointCloud<pcl::Normal>);
  ...
  for (i = 0; cloud->points.size() > 0.4 * nr_points; i++) {
    ...
    if (inliers->indices.size() == 0) {
      // std::cout << "Could not estimate a model for the given dataset." <<
      // std::endl;
      break;
    }
...
int ClusterExtraction::getPlanes(
    const pcl::PointCloud<PoinT>::ConstPtr& _cloud) {
  pcl::SACSegmentation<PoinT> seg;
  pcl::search::KdTree<PoinT>::Ptr tree(new pcl::search::KdTree<PoinT>());
